...
const Blogs = () => {
  // 1. MOCK DATA (You will later replace this with real data from your backend)
  const blogPosts = [
    {
      id: 1,
    ...
    <div className="bg-gray-50 min-h-screen pb-20">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mt-12 md:mt-16">
        ...
        <div className="text-center mb-12">
          ...
          <div className="w-20 h-1 bg-[#FF5733] mx-auto mt-3 rounded-full"></div>
          <p className="text-gray-600 mt-4 max-w-2xl mx-auto">
            Tips, guides, and stories from the heart of Africa.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {blogPosts.map((post) => (
            <article
              key={post.id}
              className="bg-white rounded-xl shadow-lg overflow-hidden hover:shadow-2xl transition-shadow duration-300 flex flex-col h-full group"
            >
              {/* Image Container */}
              <div className="relative h-56 overflow-hidden">
                ...
                />
                {/* Category Badge */}
                <div className="absolute top-4 left-4 bg-[#FF5733] text-white text-xs font-bold px-3 py-1 rounded-full shadow-md uppercase tracking-wide">
                  {post.category}
                </div>
              </div>
              ...
              <div className="p-6 flex flex-col flex-grow">
                ...
                {/* Title */}
                <h3 className="text-xl font-bold text-gray-800 mb-3 line-clamp-2 group-hover:text-[#FF5733] transition-colors font-['Playfair_Display']">
                ...
                <Link
                  to={`/blogs/${post.id}`}
                  className="inline-flex items-center text-[#FF5733] font-semibold text-sm hover:underline mt-auto"
                >
        ...
        {/* Load More Button (Optional Placeholder) */}
        <div className="mt-16 text-center">
          <button className="border-2 border-gray-800 text-gray-800 px-8 py-3 rounded-full font-semibold hover:bg-gray-800 hover:text-white transition-all duration-300">
            Load More Posts
          </button>
        </div>
      </div>
